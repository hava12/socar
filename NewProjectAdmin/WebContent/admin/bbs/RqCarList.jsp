          ...
          <div class="table-responsive">
            ...
            <table class="table table-bordered table-striped">
              ...
              <thead>
                ...
                <tr>
                  <th style="text-align: center; width: 10%">차량신청게시판코드</th>
              ...
              <tbody>
                ...
                <c:if test="${not listSimpleMem}">
                		<c:forEach items="${list}" var="item">
	                		<tr>
	                					<td>${item.rqc_code}</td>	           	                					
						                <td>${item.rqc_content}</td>
						                <td>${item.smem_id}</td>
						                <td>${item.rqc_like_count}</td>
						                <td>${item.rqc_dis_count}</td>
						                <td>${item.rqc_date}</td>
						                <td><button type="button" class="btn btn-info" onclick="goDelete('${item.rqc_code}')">삭제</button></td>
	                		</tr>
                		</c:forEach>
                </c:if>
              </tbody>
