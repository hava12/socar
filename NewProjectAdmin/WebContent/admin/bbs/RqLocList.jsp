...
<head>
   ...
   <link rel="shortcut icon" href="${pageContext.request.contextPath}/bootstrap/img/favicon.png"/>
	...
	<script>
		function goDelete(rql_code){
			if(confirm("정말로 삭제하시겠습니까?")){
				location.href="<c:url value='/Bbs/RqLocDelete.do?rql_code="+rql_code+"'/>";
				}			
		}
		...
		function find(sel) {
			...
			if (sel == "id") {
				...
				menu.innerHTML = "신청 내용 검색<span class='fa fa-caret-down' value='rql_content'></span>";
				abc.value="rql_content"
			}
		}
	</script>
  </head>
          ...
          <div class="table-responsive">
            ...
            <table class="table table-bordered table-striped">
              ...
              <thead>
                ...
                <tr>
                  <th style="text-align: center; width: 10%">쏘카존신청게시판코드</th>
                  <th style="text-align: center; width: 35%">신청내용</th>
                  <th style="text-align: center; width: 15%">멤버아이디</th>
                  <th style="text-align: center; width: 10%">좋아요</th>
                  <th style="text-align: center; width: 10%">싫어요</th>
              ...
              <tbody>
                ...
                <c:if test="${not listSimpleMem}">
                		<c:forEach items="${list}" var="item">
	                		<tr>
	                					<td>${item.rql_code}</td>	           	                					
						                <td>${item.rql_content}</td>
						                <td>${item.smem_id}</td>
						                <td>${item.rql_like_count}</td>
						                <td>${item.rql_dis_count}</td>
						                <td>${item.rql_date}</td>
						                <td><button type="button" class="btn btn-info" onclick="goDelete('${item.rql_code}')">삭제</button></td>
	                		</tr>
                		</c:forEach>
                </c:if>
              </tbody>
             ...
             <div class="input-group margin">
                 <div class="input-group-btn">
                     ...
                     <ul id="menu" class="dropdown-menu" name="searchColumn">
                         ...
                         <li><a onclick="find('name')">신청 내용 검색</a></li>
                     </ul>
                 </div>
